...
robot.on("messageDelete", (msg) => {
  if (typeof msg.content !== 'undefined'){
	...
	console.log('Кинул в лс удаленное сообщение')
      robot.users.get("331465338669629450").send(`Удалено сообщение от ${msg.author.username}, написанное ${date.toUTCString()}: "${msg.content}". К сообщению было что-то прикреплено.`);
    } else {
      robot.users.get("331465338669629450").send(`Удалено сообщение от ${msg.author.username}, написанное ${date.toUTCString()}: "${msg.content}".`);
    };
  } else {
    robot.users.get("331465338669629450").send("Удалено сообщение.");
  };
});
